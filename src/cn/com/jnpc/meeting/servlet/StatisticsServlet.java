...
public class StatisticsServlet extends BaseServlet {
    ...
    public String count() {
        ...
        if (isEmpty(day)) {
            day = DateUtil.getCurrentDate("dd");//此处请参阅API java.text.SimpleDateFormat
        }
        List<Counter> counters = meetingDao.meetingCount(Integer.parseInt(year), Integer.parseInt(month),Integer.parseInt(day));
        Date curDate = DateUtil.getDate(Integer.parseInt(year),Integer.parseInt(month), 0);//当前月的最后一天
        Date queryDate = DateUtil.getDate(Integer.parseInt(year), Integer.parseInt(month)-1, Integer.parseInt(day));//当前月的查询当天。
        ...
        request.setAttribute("queryend", DateUtil.dateToString(queryend,"yyyy-MM-dd"));
        request.setAttribute("querynextend", DateUtil.dateToString(nextDate,"yyyy-MM-dd"));
        request.setAttribute("querynextbegin", DateUtil.dateToString(firstNextDate,"yyyy-MM-dd"));
