...
public class MeetingServlet extends BaseServlet {
	...
	public String compareM(Meeting old,Meeting nw){
	    ...
	    try{
    	    String oldstartTime=old.getStarttime()==null?"":DateUtil.dateToString(old.getStarttime(),"YYYY-MM-dd HH:mm:ss");//会议起始时间
    	    String newstartTime=nw.getStarttime()==null?"":DateUtil.dateToString(nw.getStarttime(),"YYYY-MM-dd HH:mm:ss");
    	    String oldendTime=old.getEndtime()==null?"":DateUtil.dateToString(old.getEndtime(),"YYYY-MM-dd HH:mm:ss");//会议结束时间
    	    String newendTime=nw.getEndtime()==null?"":DateUtil.dateToString(nw.getEndtime(),"YYYY-MM-dd HH:mm:ss");
    	    String oldcontent=old.getContent()==null?"":old.getContent();//会议内容
    	    ...
    	    String oldpresider=old.getPresider()==null?"":old.getPresider();//会议主持人
    	    String newpresider=nw.getPresider()==null?"":nw.getPresider();
    	    String oldcontact=old.getContact()==null?"":old.getContact();//联系人
    	    String newcontact=nw.getContact()==null?"":nw.getContact();
    	    String oldcontactphone=old.getContactphone()==null?"":old.getContactphone();//联系人电话
    	    String newcontactphone=nw.getContactphone()==null?"":nw.getContactphone();
    	    String oldaddress=old.getAddress1()==null?"":old.getAddress1();//会议地址
    	    String newaddress=nw.getAddress1()==null?"":nw.getAddress1();
    	    String oldfdepart=old.getFdepart()==null?"":old.getFdepart();//外部参会人员
    	    String newfdepart=nw.getFdepart()==null?"":nw.getFdepart();
    	    if(!oldleader.equals(newleader)){
    		bf.append("会议领导由原 "+oldleader+" 变更为 <font color=red >"+newleader+"</font> <br/>");
    	    }if(oldstartTime!=null && newstartTime!=null && !oldstartTime.equals(newstartTime)){
    		bf.append("会议开始时间由原 "+oldstartTime+" 变更为 <font color=red >"+newstartTime+"</font> <br/>");
    	    }if(oldendTime!=null && newendTime!=null && !oldendTime.equals(newendTime)){
    		bf.append("会议结束时间由原 "+oldendTime+" 变更为 <font color=red >"+newendTime+"</font> <br/>");
    	    }if(!oldaddress.equals(newaddress)){
    		bf.append("会议地址由原 "+oldaddress+" 变更为 <font color=red >"+newaddress+"</font> <br/>");
    	    }if(!olddepart.equals(newdepart)){
    		bf.append("参加会议部门/人员由原 "+olddepart+" 变更为 <font color=red >"+newdepart+"</font> <br/>");
    	    }if(!oldfdepart.equals(newfdepart)){
    		bf.append("参加会议外部人员由原 "+oldfdepart+" 变更为 <font color=red >"+newfdepart+"</font> <br/>");
    	    }if(!oldremark.equals(newremark)){
    		bf.append("会议说明由原 "+oldremark+" 变更为 <font color=red >"+newremark+"</font> <br/>");
    	    }if(!oldpresider.equals(newpresider)){
    		bf.append("会议主持人由原 "+oldpresider+" 变更为 <font color=red >"+newpresider+"</font> <br/>");
    	    }if(!oldcontactphone.equals(newcontactphone)){
    		bf.append("会议联系人电话由原 "+oldcontactphone+" 变更为 <font color=red >"+newcontactphone+"</font> <br/>");
    	    }if(!oldcontact.equals(newcontact)){
    		bf.append("会议联系人由原 "+oldcontact+" 变更为 <font color=red >"+newcontact+"</font> <br/>");
    	    }if(!oldcontent.equals(newcontent)){
    		bf.append("会议内容由原 "+oldcontent+" 变更为 <font color=red >"+newcontent+"</font> <br/>");
    	    }
	    	bf.append("<br/><br/>");
	    	return bf.toString();
	    }catch(Exception e){
...
//					"yyyy-MM-dd");
		...
		if(!(i>0)){
		    meetings = meetingDao.getMeetingByRoomAndTime(roomid,
				starttime, endtime);
		}
